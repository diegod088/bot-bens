...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Add buttons with better UX (multi-language)
    keyboard = [
        [InlineKeyboardButton(get_msg("btn_download_now", lang), callback_data="start_download")],
        [
            InlineKeyboardButton(get_msg("btn_how_to_use", lang), callback_data="show_guide"),
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user['premium']:
        ...
        # Contenido bloqueado
        message += f"🔒 *Música:* `{required}`\n"
        message += f"🔒 *APK:* `{required}`\n\n"
        
        upgrade_title = "UPGRADE TO PREMIUM" if lang == 'en' else "MEJORA A PREMIUM"
        only_label = "Only" if lang == 'en' else "Solo"
        stars_label = "stars" if lang == 'en' else "estrellas"
        daily_label = "daily" if lang == 'en' else "diarios"
        unlimited_label = "Unlimited photos" if lang == 'en' else "Fotos ilimitadas"
        songs_label = "songs daily" if lang == 'en' else "canciones diarias"
        
        message += "```\n"
        message += "┌─────────────────────────────┐\n"
        message += f"│    💎 {upgrade_title:<20} │\n"
        message += "└─────────────────────────────┘\n"
        message += "```\n"
        message += f"⭐ *{only_label} {PREMIUM_PRICE_STARS} {stars_label}*\n"
        message += f"✅ 50 videos {daily_label}\n"
        message += f"✅ {unlimited_label}\n"
        message += f"✅ 50 {songs_label}\n"
        message += f"✅ 50 APK {daily_label}\n\n"
    
    # Footer
    footer_text = "Use /premium to upgrade" if lang == 'en' else "Usa /premium para mejorar"
    message += "```\n"
    message += "╔═══════════════════════════════╗\n"
    message += f"║  {footer_text:<28} ║\n"
    message += "╚═══════════════════════════════╝\n"
    message += "```"
...
def main():
    """Start the bot"""
    # Create application with increased timeouts and retry settings
    from telegram.request import HTTPXRequest
    
    # Configure HTTP request with longer timeouts
    request = HTTPXRequest(
        connection_pool_size=8,
        connect_timeout=30.0,  # Aumentado de 5.0 a 30.0
        read_timeout=30.0,     # Aumentado de 5.0 a 30.0
        write_timeout=30.0,    # Aumentado de 5.0 a 30.0
        pool_timeout=30.0      # Aumentado de 1.0 a 30.0
    )
    ...
    application.add_handler(CommandHandler("adminstats", adminstats_command))  # Panel de administración
